...
trait Evaluator {
  self : Eval.ConcreteGraphTypes =>
	...
	=
	{
	  ...
	  arg.graph.vertexSet.asScala.toList match {  // BugBug -- lgm
	...
	= {
	    if ( !(y == arg) ) {
	      for(
                v <- y.graph.vertexSet.asScala //scala.collection.jcl.Conversions.convertSet()
	      ) yield arg.graph.addVertex( v ) ;
	      for(
                e <- y.graph.edgeSet.asScala //scala.collection.jcl.Conversions.convertSet( )		
	    ...
	    for(
              v <- x.graph.vertexSet.asScala
              //scala.collection.jcl.Conversions.convertSet( )     
	    ) yield arg.graph.addVertex( v ) ;
	    for(
              e <- x.graph.edgeSet.asScala //scala.collection.jcl.Conversions.convertSet( )	      
	    ) yield arg.graph.addEdge(
